...
class Controller:
   ...
   #    reactor pressure control (reactor pressure -> A and C feed)
        if self.reset_times[3] == 0 or time - self.reset_times[3] > 1.:
            ...
            self.xmv[3] += self.gain[6] * ((err - self.err[6]) + err * self.delta_t * 60. / self.taui[6])
            self.err[6] = err
            self.xmv[3] = np.clip(self.xmv[3], 0., 100.)
        # else:
        #     print("xmv[3] open")
   #    reactor temperature control (reactor temperature -> reactor coolant flow)
        if self.reset_times[9] == 0 or time - self.reset_times[9] > 1.:
            err = self.setpt[0]-xmeas[9]
            self.xmv[9] += self.gain[0]*((err-self.err[0])+err*self.delta_t*60./self.taui[0])
            self.err[0] = err
            self.xmv[9] = np.clip(self.xmv[9], 0., 100.)
        # else:
        #     print("xmv[9] open")
   #    reactor level control (reactor level -> E feed?)
        if self.reset_times[1] == 0 or time - self.reset_times[1] > 1.:
            err = self.setpt[1]-self.fxmeas[8]
            self.xmv[1] += self.gain[1]*((err-self.err[1])+err*self.delta_t*60./self.taui[1])
            self.err[1] = err
            self.xmv[1] = np.clip(self.xmv[1], 0., 100.)
        # else:
        #     print("xmv[1] open")
   #    product separator level control (sep level -> condensor coolant flow)
        if self.reset_times[10] == 0 or time - self.reset_times[10] > 1.:
            err = self.setpt[2]-self.fxmeas[12]
            self.xmv[10] += self.gain[2]*((err-self.err[2])+err*self.delta_t*60./self.taui[2])
            self.err[2] = err
            self.xmv[10] = np.clip(self.xmv[10], 0., 100.)
        # else:
        #     print("xmv[10] open")
   #    stripper level control (strip level -> sep pot flow)
        if self.reset_times[6] == 0 or time - self.reset_times[6] > 1.:
            err = self.setpt[3]-self.fxmeas[15]
            self.xmv[6] += self.gain[3]*((err-self.err[3])+err*self.delta_t*60./self.taui[3])
            self.err[3] = err
            self.xmv[6] = np.clip(self.xmv[6], 0., 100.)
        # else:
        #     print("xmv[6] open")
   #    stripper underflow control (strip underflow -> product flow)
        if self.reset_times[7] == 0 or time - self.reset_times[7] > 1.:
            err = self.setpt[4]-xmeas[17]
            self.xmv[7] += self.gain[4]*((err-self.err[4])+err*self.delta_t*60./self.taui[4])
            self.err[4] = err
            self.xmv[7] = np.clip(self.xmv[7], 0., 100.)
        # else:
        #     print("xmv[7] open")
   #    g/h ratio control (ratio -> D feed?)
        if self.reset_times[0] == 0 or time - self.reset_times[0] > 1.:
            err = self.setpt[5]-xmeas[42]
            self.xmv[0] += self.gain[5]*((err-self.err[5])+err*self.delta_t*60./self.taui[5])
            self.err[5] = err
            self.xmv[0] = np.clip(self.xmv[0], 0., 100.)
        # else:
        #     print("xmv[0] open")
   #    purge gas b component control (b -> purge)
        if self.reset_times[5] == 0 or time - self.reset_times[5] > 1.:
            err = self.setpt[7]-xmeas[30]
            self.xmv[5] += self.gain[7]*((err-self.err[7])+err*self.delta_t*60./self.taui[7])
            self.err[7] = err
            self.xmv[5] = np.clip(self.xmv[5], 0., 100.)
        # else:
        #     print("xmv[5] open")
   #    reactor feed a component control (reactor feed A -> A feed?)
        if self.reset_times[2] == 0 or time - self.reset_times[2] > 1.:
            err = self.setpt[8]-xmeas[23]
            self.xmv[2] += self.gain[8]*((err-self.err[8])+err*self.delta_t*60./self.taui[8])
            self.err[8] = err
