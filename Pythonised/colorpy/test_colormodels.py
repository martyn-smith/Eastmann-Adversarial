...
def test_xyz_rgb(verbose=1):
    ...
    def test_A(xyz0, tolerance=1.0e-10, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_xyz_rgb.test_A() : xyz0 = %s, rgb(xyz0) = %s, xyz(rgb(xyz0)) = %s, rgb(xyz(rgb(xyz0))) = %s, errors = (%g, %g), %s"
    ...
    test_eye0 = numpy.dot(
        colormodels.rgb_from_xyz_matrix, colormodels.xyz_from_rgb_matrix
    )
    test_eye1 = numpy.dot(
        colormodels.xyz_from_rgb_matrix, colormodels.rgb_from_xyz_matrix
    )
    ...
    msg = "test_xyz_rgb() : %d tests passed, %d tests failed" % (num_passed, num_failed)
    print(msg)
...
def test_xyz_irgb(verbose=1):
    ...
    for i in range(0, 100):
        ...
        if (irgb0[0] != irgb1[0]) or (irgb0[1] != irgb1[1]) or (irgb0[2] != irgb1[2]):
            raise ValueError
        irgbs0 = colormodels.irgb_string_from_rgb(colormodels.rgb_from_xyz(xyz0))
...
def calc_L_LUM_C():
    ...
    wanted = (
        colormodels.L_LUM_A * math.pow(colormodels.L_LUM_CUTOFF, 1.0 / 3.0)
        - colormodels.L_LUM_B
    ) / colormodels.L_LUM_CUTOFF
...
def test_L_luminance(verbose=1):
    ...
    def test_A(y0, tolerance=1.0e-13, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_L_luminance.test_A() : y0 = %g (%s), L(y0) = %g, y(L(y0)) = %g, error = %g, %s"
    ...
    def test_B(L0, tolerance=1.0e-10, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_L_luminance.test_B() : L0 = %g (%s), y(L0) = %g, L(y(L0)) = %g, error = %g, %s"
...
def test_uv_primes(verbose=1):
    ...
    def test_A(xyz0, tolerance=0.0, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_uv_primes.test_A() : xyz0 = %s, (up,vp) = (%g,%g), xyz(up,vp) = %s, error = %g, %s"
    ...
    def test_B(up0, vp0, y0, tolerance=0.0, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_uv_primes.test_B() : (up0,vp0,y0) = (%g,%g,%g), xyz (up0,vp0,y0) = %s, (up,vp)(xyz) = (%g,%g), error = %g, %s"
...
def calc_LAB_F_A():
    ...
    wanted = (
        math.pow(colormodels.L_LUM_CUTOFF, 1.0 / 3.0) - colormodels.LAB_F_B
    ) / colormodels.L_LUM_CUTOFF
...
def test_Lab_f(verbose=1):
    ...
    def test_A(t0, tolerance=1.0e-13, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_Lab_f.test_A() : t0 = %g (%s), f(t0) = %g, t(f(t0)) = %g, error = %g, %s"
    ...
    def test_B(f0, tolerance=1.0e-10, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_Lab_f.test_B() : f0 = %g (%s), t(f0) = %g, f(t(f0)) = %g, error = %g, %s"
    ...
    msg = "test_Lab_f() : %d tests passed, %d tests failed" % (num_passed, num_failed)
    print(msg)
...
def test_xyz_luv(verbose=1):
    ...
    def test_A(xyz0, tolerance=1.0e-10, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_xyz_luv.test_A() : xyz0 = %s, luv(xyz0) = %s, xyz(luv(xyz0)) = %s, luv(xyz(luv(xyz0))) = %s, errors = (%g, %g), %s"
    ...
    msg = "test_xyz_luv() : %d tests passed, %d tests failed" % (num_passed, num_failed)
    print(msg)
...
def test_xyz_lab(verbose=1):
    ...
    def test_A(xyz0, tolerance=1.0e-10, verbose=1):
        ...
        if passed:
            ...
            status = "FAILED"
        msg = (
            "test_xyz_lab.test_A() : xyz0 = %s, lab(xyz0) = %s, xyz(lab(xyz0)) = %s, lab(xyz(lab(xyz0))) = %s, errors = (%g, %g), %s"
    ...
    msg = "test_xyz_lab() : %d tests passed, %d tests failed" % (num_passed, num_failed)
    print(msg)
