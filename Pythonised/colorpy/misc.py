...
def colorstring_patch_plot(colorstrings, color_names, title, filename, num_across=6):
    ...
    plots.rgb_patch_plot(
        rgb_colors, color_names, title, filename, num_across=num_across
    )
...
def MacBeth_ColorChecker_patch_plot():
    ...
    plots.xyz_patch_plot(
        xyz_colors, color_names, "MacBeth ColorChecker Chart", "MacBeth"
    )
...
def spectral_colors_patch_plot():
    ...
    xyzs = ciexyz.get_normalized_spectral_line_colors(
        brightness=1.0, num_purples=0, dwl_angstroms=10
    )
    plots.xyz_patch_plot(
...
def spectral_colors_plus_purples_patch_plot():
    """Colors of the pure spectral lines plus purples."""
    xyzs = ciexyz.get_normalized_spectral_line_colors(
        brightness=1.0, num_purples=200, dwl_angstroms=10
    )
    plots.xyz_patch_plot(
        xyzs,
        None,
...
def perceptually_uniform_spectral_colors():
    ...
    # get pure colors
    xyzs = ciexyz.get_normalized_spectral_line_colors(
        brightness=1.0, num_purples=200, dwl_angstroms=1
    )
...
def figures():
    ...
    colorstring_patch_plot(
        primary_colors, primary_names, "Primary Colors", "primary", num_across=4
    )
